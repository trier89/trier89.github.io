...
사용법은 아주 간단해요. **생년월일 8자리(예: 19880818)**를 숫자로 입력하고 버튼만 누르면 돼요. 그러면 태어난 요일, 그날 세상에서 일어난 역사적 사건, 그리고 나와 생일이 같은 유명인을 '요즘 많이 찾아보는 순서'로 보여줘요.

- **부모님·친구 생일**을 넣어보면 대화 소재가 돼요. "네 생일에 이런 일이 있었대!" 하고 알려주면 반응이 좋아요.
- 결과 아래 **공유 버튼**을 누르면 요약 문구가 만들어져서, 메신저나 SNS로 바로 나눌 수 있어요.
- 나와 생일이 같은 유명인은 위키백과 문서 조회수를 기준으로 정렬하니, 시기에 따라 순서가 조금씩 바뀔 수 있어요.

## 함께 보면 좋은 것

- 태어난 날을 알았다면, **만 나이·띠·별자리·생일 D-day**가 궁금해지죠? [만나이 계산기](/tools/age-calculator/)로 한 번에 확인해 보세요.
- 특정 날짜까지 며칠 남았는지 세고 싶다면 [디데이 계산기](/tools/dday/)가 편해요.
- 생년월일 데이터는 어디에도 저장되지 않고 **여러분 브라우저 안에서만** 처리되니 안심하고 넣어도 돼요.

## 자주 묻는 질문

**Q. 생년월일은 어떻게 입력하나요?**
연·월·일을 붙여서 숫자 8자리로 넣으면 돼요. 예를 들어 1988년 8월 18일이면 19880818처럼 입력하면 자동으로 점이 찍혀요.

**Q. 입력한 생일이 저장되나요?**
아니요. 모든 조회는 여러분의 브라우저에서 직접 이루어지고, 생년월일은 서버로 전송되거나 저장되지 않아요.

**Q. 유명인 순서는 무슨 기준인가요?**
각 인물의 한국어 위키백과 문서가 최근 60일간 얼마나 조회됐는지를 기준으로 정렬해요. 요즘 많이 찾아보는 인물이 위로 올라와요.

**Q. 내 생일인데 사건이나 유명인이 안 나와요.**
위키백과의 해당 날짜 문서에 정리된 내용을 불러오는 방식이라, 날짜에 따라 데이터가 적을 수 있어요. 잠시 후 다시 시도하면 불러와지는 경우도 있어요.

**Q. 만 나이나 띠도 여기서 볼 수 있나요?**
이 도구는 '그날의 이야기'에 집중해요. 만 나이·띠·별자리는 [만나이 계산기](/tools/age-calculator/)에서 확인할 수 있어요.
