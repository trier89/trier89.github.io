...
띠는 **12년 주기**로 돌아와요. 쥐·소·호랑이·토끼·용·뱀·말·양·원숭이·닭·개·돼지 순서로, 태어난 해에 따라 하나가 정해지죠. 그래서 12살 차이가 나는 사람끼리는 같은 띠예요.

별자리는 **태어난 월·일**로 정해져요. 같은 별자리라도 해가 바뀌면 나이는 달라지지만 별자리는 그대로예요. 위 계산기는 생년월일만 넣으면 띠와 별자리를 한 번에 보여드려요.

- **만 나이·연 나이·세는 나이**가 헷갈릴 땐 위 표에서 세 가지를 한눈에 비교해 보세요.
- 서류에 "만 나이"라고 적혀 있으면 위 계산기의 큰 숫자를 그대로 쓰면 돼요.
- 기준일을 바꾸면 특정 날짜에 내가 몇 살이었는지도 확인할 수 있어요(예: 입사일, 시험일 기준 나이).

## 자주 묻는 질문

**Q. 생일이 오늘이면 나이가 어떻게 되나요?**
만 나이는 생일 당일에 한 살 올라가요. 그래서 오늘이 생일이면 이미 한 살 더한 값이 표시돼요.

**Q. 2월 29일생은 나이 계산이 다른가요?**
만 나이 계산 자체는 같아요. 다만 평년에는 2월 29일이 없어 법적으로 3월 1일에 나이를 더하는 것으로 보는 경우가 많아요.

**Q. 다음 생일까지 며칠 남았는지도 알 수 있나요?**
네, 위 결과의 "다음 생일까지" 항목에 D-day로 나와요. 특정 목표일까지 세고 싶다면 [디데이 계산기](/tools/dday/)를 써 보세요.

**Q. 내가 태어난 날엔 무슨 일이 있었을까요?**
[내 생일엔 무슨 일이?](/tools/my-birthday/)에서 생일 관련 이야기를 확인해 보세요.
