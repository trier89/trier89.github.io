...
프로젝트 마감이나 연차 계획을 세울 때는 그냥 며칠이 아니라 **실제로 일하는 날**이 중요하죠. 위 계산기의 "실근무일"은 평일(월~금)에서 그 사이에 낀 공휴일을 뺀 값이라, 달력을 일일이 세지 않아도 돼요.

- **계약·정산 기간**: 두 날짜 사이 총 일수(양 끝 포함)로 위약금이나 이자 계산의 기준을 잡을 수 있어요.
- **여행·휴가 계획**: 연휴에 며칠 붙이면 며칠을 쉴 수 있는지, 공휴일이 주말과 겹쳐 손해인지 미리 확인할 수 있어요.
- **디데이가 필요하면**: 특정 날짜까지 며칠 남았는지만 알고 싶다면 [디데이 계산기](/tools/dday/)가 더 편해요.

## 공휴일 반영 범위

공휴일은 **2025·2026년 대한민국 법정공휴일과 대체공휴일**을 반영했어요(관보·월력요항 기준). 설날·추석처럼 음력으로 정해지는 명절과 대체공휴일까지 포함돼 있어요. 다만 회사 창립기념일이나 개별 약정 휴무는 반영되지 않으니, 그런 날은 결과에서 따로 빼서 생각하세요.

## 자주 묻는 질문

**Q. "며칠 후"에 음수를 넣으면 어떻게 되나요?**
음수(예: -100)를 넣으면 기준 날짜에서 그만큼 **이전** 날짜를 알려줘요. 과거 어느 날이 무슨 요일이었는지 확인할 때 편해요.

**Q. 총 일수와 실근무일이 왜 다른가요?**
총 일수는 주말·공휴일을 모두 포함한 달력상 날짜 수예요. 실근무일은 여기서 주말과 공휴일을 빼서, 실제로 근무하는 평일만 센 값이에요.

**Q. 두 날짜 사이 일수에 시작일과 끝일이 다 포함되나요?**
근무일 표의 "총 일수(양 끝 포함)"는 양쪽 날짜를 모두 세요. 반면 상단의 큰 숫자는 두 날짜 사이의 간격(차이)이라 하루 적게 나올 수 있어요.

**Q. 만 나이나 기념일도 계산할 수 있나요?**
생년월일 기준 나이는 [만나이 계산기](/tools/age-calculator/), 100일·1주년 같은 기념일은 [디데이 계산기](/tools/dday/)를 이용하세요.
