...
버튼 하나로 **이번 주 추천 5게임**이 자동으로 만들어져요. 아래에서 몇 가지를 조절할 수 있어요.

- **꼭 넣을 번호**: 마음에 두고 있는 번호(생일, 좋아하는 숫자 등)를 최대 6개까지 골라두면 모든 게임에 반드시 포함돼요.
- **확률 기반 번호 수(K)**: 6개 중 몇 개를 '과거 출현 빈도 블렌드'로 뽑을지 정해요. K=0이면 완전 무작위예요.
- **최근 표본(N)**: 최근 몇 회차의 통계를 얼마나 비중 있게 볼지 정하는 값이에요.

같은 설정·같은 입력이면 **추천 결과가 항상 똑같이** 나와요. 그래서 "아까 그 번호"를 다시 확인하기 편해요. 아래쪽에는 역대 누적 기준 핫넘버·콜드넘버, 홀짝·합계·구간 분포 같은 통계도 재미로 볼 수 있게 정리해 뒀어요.

## 솔직하게 — 이건 예측 도구가 아니에요

로또는 매 회차가 **완전히 독립적인 무작위 추첨**이에요. 지난주에 많이 나온 번호(핫넘버)든 오래 안 나온 번호(콜드넘버)든, 다음 추첨에서 나올 확률은 **똑같아요**. 1등에 당첨될 확률은 45개 중 6개를 맞히는 조합이라 **1/8,145,060**로 고정돼 있고, 어떤 번호 선택법으로도 이 확률은 바뀌지 않아요.

- 이 생성기는 **번호를 대신 골라주는 편의·재미 도구**예요. 당첨 확률을 높여주지 않아요.
- 화면의 "요약 실적"이나 "최고 K·N 자동 찾기"는 **과거 회차에 맞춰본 결과**일 뿐이라, 다음 회차 당첨과는 무관해요(과최적화).
- 로또는 잃어도 괜찮은 금액 안에서, **가벼운 재미로만** 즐겨주세요.

번호를 무작위로 정하고 싶을 때는 [무작위 뽑기](/tools/random-picker/)도, 오늘의 기분 전환용으로는 [오늘의 운세](/tools/horoscope/)도 함께 즐겨보세요.

## 자주 묻는 질문

**Q. 이 번호로 사면 당첨 확률이 올라가나요?**
아니요. 로또는 무작위라 어떤 번호 조합이든 1등 확률은 1/8,145,060으로 같아요. 이 도구는 번호를 편하게 골라주는 재미용이에요.

**Q. 핫넘버·콜드넘버가 다음에 나올 확률이 더 높나요?**
아니요. 각 추첨은 독립적이라 과거에 자주/드물게 나온 것과 다음 결과는 아무 관계가 없어요. 통계는 지나온 기록을 보는 재미로만 봐주세요.

**Q. '최고 K·N 자동 찾기'는 뭔가요?**
과거 회차들을 대상으로 어떤 설정이 성적이 좋았는지 뒤돌아본 것이에요. 과거에 잘 맞았다는 뜻일 뿐, 미래 당첨을 보장하지 않아요.

**Q. 같은 설정인데 왜 항상 같은 번호가 나오나요?**
결정론적으로 설계했기 때문이에요. 다른 조합을 원하면 '꼭 넣을 번호'나 K·N 설정을 바꿔보세요.

**Q. 몇 게임까지 만들 수 있나요?**
한 번에 5게임을 추천해요. 설정을 바꾸면 그에 맞는 새로운 5게임이 다시 만들어져요.
