...
const _pinIcons={};
function catPin(cat,closed){
 const key=cat+(closed?'_c':'');
 if(_pinIcons[key])return _pinIcons[key];
 _pinIcons[key]=L.icon({iconUrl:'mk-'+cat+'.png',iconSize:[34,44],iconAnchor:[17,42],tooltipAnchor:[0,-38],className:closed?'mkclosed':''});
 return _pinIcons[key];
}
// 업종 영업시간(분): 없으면 24시간 취급
const HRS={starbucks:[7*60,22*60],mart:[10*60,24*60],dept:[630,20*60],station:[330,24*60],linked:[330,24*60]};
function isClosed(cat){const h=HRS[cat];if(!h)return false;const n=new Date();const m=n.getHours()*60+n.getMinutes();return m<h[0]||m>=h[1];}
function hrsLabel(cat){const h=HRS[cat];if(!h)return '';const f=x=>('0'+Math.floor((x%1440)/60)).slice(-2)+':'+('0'+(x%60)).slice(-2);return f(h[0])+'–'+(h[1]>=1440?'24:00':f(h[1]));}
function hrsRow(cat){if(!HRS[cat])return '';const cl=isClosed(cat);return '<div class="kv"><b>'+t('openHr')+'</b><span>'+hrsLabel(cat)+' · <b style="color:'+(cl?'#e0392f':'#2f9e5b')+'">'+(cl?t('closedNow'):t('openNow'))+'</b></span></div>';}
...
async function renderLine(){
 ...
 el.querySelectorAll('text').forEach(tx=>{
  ...
  const st=byname[nm];
  if(st){ tx.setAttribute('data-stn',st.id); tx.style.cursor='pointer'; tx.setAttribute('pointer-events','all'); }
  else if(/^[가-힣][가-힣0-9·().]{1,9}$/.test(nm)){ tx.setAttribute('data-noinfo',nm); tx.style.cursor='pointer'; tx.setAttribute('pointer-events','all'); }
 });
 _svgPanZoom(el);
...
function showComingSoon(name){
 openSheet('<div class="row"><h2>'+(name||'')+'</h2></div><div class="card" style="text-align:center;padding:22px 12px"><div style="font-size:30px">🚧</div><div style="font-weight:800;margin-top:6px;color:#2b2f36">'+t('comingSoon')+'</div><div class="hint" style="margin-top:4px">'+t('comingSoonSub')+'</div></div>');
}
// GPU 가속 팬/줌 (viewBox 재렌더 대신 CSS transform) — 큰 SVG 이동 랙 해결
function _svgPanZoom(svg){
 ...
 svg.style.cssText='display:block;background:#fff;position:absolute;left:0;top:0;width:'+MW+'px;height:'+MH+'px;transform-origin:0 0;will-change:transform;touch-action:none';
 const hr=host.getBoundingClientRect();
 const s0=Math.min(hr.width/MW,hr.height/MH)||0.5;
 let s=s0,tx=(hr.width-MW*s)/2,ty=(hr.height-MH*s)/2;
 const apply=()=>{svg.style.transform='translate('+tx.toFixed(1)+'px,'+ty.toFixed(1)+'px) scale('+s.toFixed(4)+')';};
 apply();
 const pts=new Map(); let pd=0,lx=0,ly=0,moved=false;
 const zoomAt=(cx,cy,f)=>{const h2=host.getBoundingClientRect();const px=cx-h2.left,py=cy-h2.top;const ns=Math.min(s0*12,Math.max(s0*0.85,s*f));const k=ns/s;tx=px-(px-tx)*k;ty=py-(py-ty)*k;s=ns;apply();};
 ...
 svg.addEventListener('pointermove',e=>{if(!pts.has(e.pointerId))return;const dx=e.clientX-lx,dy=e.clientY-ly;pts.set(e.pointerId,{x:e.clientX,y:e.clientY});
  if(pts.size>=2){const v=[...pts.values()];const nd=Math.hypot(v[0].x-v[1].x,v[0].y-v[1].y);if(pd)zoomAt((v[0].x+v[1].x)/2,(v[0].y+v[1].y)/2,pd/nd);pd=nd;moved=true;return;}
  if(Math.abs(dx)+Math.abs(dy)>3)moved=true;
  tx+=dx;ty+=dy;lx=e.clientX;ly=e.clientY;apply();});
 const end=e=>{pts.delete(e.pointerId);if(pts.size<2)pd=0;};
 svg.addEventListener('pointerup',end);svg.addEventListener('pointercancel',end);
 svg.addEventListener('wheel',e=>{e.preventDefault();zoomAt(e.clientX,e.clientY,e.deltaY>0?0.9:1.11);},{passive:false});
 svg.addEventListener('click',e=>{if(moved)return;const el2=e.target.closest&&e.target.closest('[data-stn],[data-noinfo]');if(!el2)return;const id=el2.getAttribute('data-stn');if(id){const st=DATA.stations.find(x=>String(x.id)===id);if(st)showStation(st);}else{showComingSoon(el2.getAttribute('data-noinfo'));}});
}

/* 시트 */
let _lgWasOn=false;
function openSheet(h){$('#sheetBody').innerHTML=h;$('#sheet').classList.add('up');var lg=$('#legend');if(lg.classList.contains('on')){_lgWasOn=true;lg.classList.remove('on');}}
function closeSheet(){const s=$('#sheet');s.style.transform='';s.classList.remove('up');if(_lgWasOn){$('#legend').classList.add('on');_lgWasOn=false;}}
...
function showStation(st){
 ...
 openSheet(`
  ...
  <div class="card" style="padding:8px 12px">${hrsRow('station')}</div>
...
function showToiletInfo(c){
 ...
 openSheet(`
  ...
  ${(c.addr||c.hr||HRS[c.cat])?`<div class="card">${c.addr?`<div class="kv"><b>${t('addr')}</b><span>${c.addr}</span></div>`:''}${hrsRow(c.cat)}</div>`:''}
